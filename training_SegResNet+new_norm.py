...
def train(model, train_loader, test_loader, loss, optim, max_epochs, model_dir, test_interval=1 , device=torch.device("cuda:2")):
    best_metric = -1
    best_metric_epoch = -1
    save_loss_train = []
    save_loss_test = []
    ...
    for epoch in range(max_epochs):
        ...
        np.save(os.path.join(model_dir, 'loss_train_Segresnet+new_norm.npy'), save_loss_train)
        
        epoch_metric_train /= train_step
        print(f'Epoch_metric: {epoch_metric_train:.4f}')
        save_metric_train.append(epoch_metric_train)
        np.save(os.path.join(model_dir, 'metric_train_Segresnet+new_norm.npy'), save_metric_train)
        ...
        if (epoch + 1) % test_interval == 0:
            ...
            with torch.no_grad():
                ...
                np.save(os.path.join(model_dir, 'loss_test_Segresnet+new_norm.npy'), save_loss_test)

                epoch_metric_test /= test_step
                print(f'test_dice_epoch: {epoch_metric_test:.4f}')
                save_metric_test.append(epoch_metric_test)
                np.save(os.path.join(model_dir, 'metric_test_Segresnet+new_norm.npy'), save_metric_test)
                ...
                if epoch_metric_test > best_metric: #Guardamos el modelo con los weights actualizados si el valor DICE mejora
                    ...
                    torch.save(model.state_dict(), os.path.join(
                        model_dir, "best_metric_model_Segresnet+new_norm.pth"))
